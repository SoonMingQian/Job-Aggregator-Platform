...
async def open_page(context, title, job_location, page_num):
    ...
    page_url = f"https://www.jobsie.ie/jobs/{title}/in-{job_location}?radius=20&page={page_num}&sort=2&action=sort_publish"
...
async def process_job_cards(page, processed_urls=None):
    ...
    jobs = []
    try:
        await page.wait_for_selector('article.res-ezkvph')
        job_cards = await page.query_selector_all('article.res-ezkvph')
